...
### Связи и Бейджи (Smart Badges)
- **Умные уведомления:** Бейджи на вкладках (Возможные, Входящие, Исходящие) появляются *только* при наличии новых событий.
- **Логика прочтения:** При переходе на вкладку бейдж исчезает и не появляется до поступления новых данных (используется `localStorage`).
- **Глобальный бейдж:** В меню "Связи" отображается количество активных вкладок (где есть новые, непросмотренные события).
- **Возможные друзья:** Реализована логика временного окна (1 минута) для взаимной видимости пользователей.
